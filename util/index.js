...
export function resolveSidebarItems (page, regularPath, site, localePath) {
  ...
  console.log('resolveSidebarItems', { page } );
  ...
  if (!sidebarConfig) {
    ...
    // console.log('sidebarConfig', config);
    if (config === 'auto') {
      return resolveHeaders(page)
    }
    return config
...
export function resolveTocItems (page, regularPath, site, localePath) {
  ...
  if (pageTocbarConfig === 'auto' || pageTocbarConfig === true) {
    // if (pageTocbarTitle) {
      // if (!pageTocbarTitle) {
      //   page.title = '';
      // } else if (pageTocbarTitle === true || pageTocbarTitle === 'auto') {
      //   page.title = page.title;
      // } else {
      //   page.title = pageTocbarTitle;
      // }
    // }
    // const group = resolveHeaders(page);
    // // console.log(group, page);
    // if ( page && page.headers && page.headers.length ) {
    //   console.log('resolveTocItems-1-1', group, page );
    //   // console.log(page.headers);
    //   return group;
    // } else {
    //   console.log('resolveTocItems-1-2', [] );
    //   return [];
    // }
  }
  ...
  if (!tocbarConfig) {
    ...
    // if (config === 'auto') {
      if (page.frontmatter.tocTitle === false) {
        newPage.title = '';
      } else if (pageTocbarTitle === 'auto' || pageTocbarTitle === true) {

      } else {
        newPage.title = pageTocbarTitle;
      }
      return resolveHeaders(newPage)
    // }
    return config
    ? config.map(item => resolveItem(item, pages, base))
    : []
